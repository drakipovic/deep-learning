...
with g2.as_default():
    ...
    for step in range(gibbs_sampling_steps):
        v3_prob = tf.sigmoid(tf.matmul(h3, w2, transpose_b=True) + vb2)
...
with g3.as_default():   
    ...
    v1_up_down_prob = tf.sigmoid(tf.matmul(h1_up_prob, w1_down) + vb1_down)
    v1_up_down = sample_prob(v1_up_down_prob)
    
    # top RBM Gibs passes
    h2_up_prob = tf.sigmoid(tf.matmul(h1_up_prob, w2a) + hb2a)
    h2_up = sample_prob(h2_up_prob)
    h4 = h2_up
    h4_prob = h2_up_prob

    for step in range(gibbs_sampling_steps):
        h1_down_prob = tf.sigmoid(tf.matmul(h4_prob, w2a, transpose_b=True) + hb1_down)
        h1_down = sample_prob(h1_down_prob)
        h4_prob = tf.sigmoid(tf.matmul(h1_down_prob, w2a) + hb2a)
        h4 = sample_prob(h4_prob)
       
    # sleep pass
    v1_down_prob = tf.sigmoid(tf.matmul(h1_down_prob, w1_down) + vb1_down)
    v1_down = sample_prob(v1_down_prob)
    h1_down_up_prob = tf.sigmoid(tf.matmul(v1_down_prob, w1_up) + hb1_up)
    h1_down_up = sample_prob(h1_down_prob)
    ...
    # top RBM update
    w2_positive_grad = tf.matmul(h1_up, h2_up)
    w2_negative_grad = tf.matmul(h1_down, h4)
