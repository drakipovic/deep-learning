...
def train(train_x, train_y, valid_x, valid_y, net, loss, config):
  ...
  for epoch in range(1, max_epochs+1):
    ...
    for i in range(num_batches):
      ...
      if i > 0 and i % 50 == 0:
        print("Train accuracy = %.2f" % (cnt_correct / float((i+1)*batch_size) * 100))
    print("Train accuracy = %.2f" % (cnt_correct / float(num_examples * 100)))
...
def evaluate(name, x, y, net, loss, config):
  ...
  valid_acc = (cnt_correct / float(num_examples)) * 100
  loss_avg /= float(num_batches)
