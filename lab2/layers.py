...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    return np.multiply(grads, self.inputs > 0)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def backward_inputs(self, x, y):
    ...
    # TODO
    return ((np.exp(x) / np.sum(np.exp(x), axis=1, keepdims=True)) - y) / x.shape[0]
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    return self.weight_decay * np.sum(np.dot(self.weights, self.weights.T))
